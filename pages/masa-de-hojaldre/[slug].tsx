...
const MasaDeHojaldre = (props: any) => {
...
export default MasaDeHojaldre


export async function getStaticPaths() {
    try {
        const slug = await fetch(`${process.env.NEXT_PUBLIC_DOMAIN_URL as string}/api/masa-de-hojaldre`)
...
export async function getStaticProps({params: {slug}}: never) {
    try {
        const info = await fetch(`${process.env.NEXT_PUBLIC_DOMAIN_URL as string}/api/masa-de-hojaldre`)
